...
def calculate_activations(thisProcess, log_file, this_autoencoder, layer_list, layer_locations, data_file, translated_source_folder, activations_source_folder, delete_existing, languages, activation_function, filerepo, layer_sparsity_type):
    ...
    try:
        ...
        if this_autoencoder == "google/gemma-3-1b-pt":  
            ...
            sae_file = f"resid_post/layer_{LAYER}_width_{WIDTH}_l0_{L0}/params.safetensors"
            ...
            pt_params = {k: v.to(torch.float32) for k, v in params.items()}

            d_model, d_sae = params["w_enc"].shape
            sae = JumpReLUSAE_2(d_model, d_sae)
            ...
            with torch.inference_mode():
                ...
                for s in range(0, len(prompts), BATCH_SIZE):
                    ...
                    for b in range(B):
                        ...
                        base = f"{sa_model.replace('/','_')}--{nf}--{s+b}--{concept}--{sae_file_tag.replace('__params.npz','').replace('resid_post__','').replace('__params.safetensors','')}"
                        out_dir_1 = activations_source_folder + '/' + activation_function 

                        if not os.path.exists(out_dir_1):
                                os.makedirs(out_dir_1)

                        out_dir = out_dir_1 + '/' + sae_file_tag
        ...
        elif this_autoencoder == "google/gemma-2-2b":
            ...
            for layer in range(12):
                ...
                if layer in layer_list:
                    ...
                    update_log(log_file, thisProcess + ": loaded blobfile " + sae_path)
                    ...
                    with open(data_file, "r", encoding="utf-8") as f:
                        ...
                        for line in lines:
                            ...
                            if p > 1:
                                ...
                                fileOut = f_sae + '--' + nf + '--' + str(p) + '--' + concept.lower() + '--layer_' + f_layer + '-' + f_conf + '--sparsity_' + sparsity
