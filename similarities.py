...
def calculate_similarities(thisProcess, log_file, ground_truth_file, languages, concept_pos_in_file, inspect, inspect_concepts, inspect_folder, similarities_folder, averages_folder, activations_folder, data_start_file, binary_target, average_function):    
    ...
    for lang in languages:
        ...
        for folder in dataFolders:
            ...
            for fname in os.listdir(folder):
                if fname.endswith(".npy") and l in fname and average_function in fname:
            ...
            if binary_target:
                ...
                for (concept1, tensor1), (concept2, tensor2) in all_combinations:
                    ...
                    if ground_truth.get((concept1, concept2),0) == 1:
                        ...
                        if cnt_negs < ground_truth_length:
                            cnt += 1
                            if cnt % 100 == 0:
                                print(cnt)
    ...
    for folder in dataFolders:
        ...
        ex_format = 'similarities'
        output_filename = os.path.join(similarities_folder, f"{ex_format}--{average_function}--en--{activation_method}--layer_{layer}--width_{width}--sparsity_{sparsity}.csv")
        # Write to CSV
        output_df = pd.DataFrame(results, columns=["concept1", "concept2", "similarity", "ground_truth"])
        output_df.to_csv(output_filename, index=False)
